...
-- They are either trigger functions, RLS helpers (invoked inside policies, not via API),
-- or admin-only routines. service_role retains access for server-side use.
DO $$
DECLARE r record;
BEGIN
  FOR r IN
