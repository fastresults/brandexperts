...
BEGIN
  ...
  SELECT id, founders_seats, founders_price_cents, cohort_seats, cohort_price_cents
    INTO c FROM public.cohorts WHERE id = _cohort_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Cohort % not found', _cohort_id; END IF;
  SELECT
  ...
  WHERE cohort_id = _cohort_id AND status IN ('paid','confirmed') AND id <> _registration_id;
  IF _requested_tier = 'founders' AND founders_taken < c.founders_seats THEN
    chosen := 'founders'; chosen_price := c.founders_price_cents;
  ELSIF cohort_taken < c.cohort_seats THEN
    chosen := 'cohort'; chosen_price := c.cohort_price_cents;
  ...
  UPDATE public.workshop_registrations
     SET status = 'paid', assigned_tier = chosen, price_paid_cents = chosen_price,
...
DECLARE
  c RECORD; paid_total integer; capacity integer; remaining integer; new_status text;
BEGIN
  SELECT founders_seats, cohort_seats INTO c FROM public.cohorts WHERE id = _cohort_id FOR UPDATE;
  IF NOT FOUND THEN RETURN; END IF;
  capacity := c.founders_seats + c.cohort_seats;
  SELECT COUNT(*) INTO paid_total FROM public.workshop_registrations
   WHERE cohort_id = _cohort_id AND status IN ('paid','confirmed');
  remaining := GREATEST(capacity - paid_total, 0);
  IF remaining = 0 THEN new_status := 'sold_out';
  ELSIF paid_total > 0 OR remaining < capacity THEN new_status := 'filling';
