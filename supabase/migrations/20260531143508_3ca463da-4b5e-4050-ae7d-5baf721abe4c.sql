...
CREATE POLICY "Cohorts are publicly readable" ON public.cohorts FOR SELECT USING (true);
CREATE POLICY "Super admins insert cohorts" ON public.cohorts FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'super_admin'));
CREATE POLICY "Super admins update cohorts" ON public.cohorts FOR UPDATE TO authenticated
  USING (has_role(auth.uid(), 'super_admin'));
CREATE POLICY "Super admins delete cohorts" ON public.cohorts FOR DELETE TO authenticated
  USING (has_role(auth.uid(), 'super_admin'));
